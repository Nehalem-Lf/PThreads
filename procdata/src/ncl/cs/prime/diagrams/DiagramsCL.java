...
public class DiagramsCL {
	...
	private static BarChart createChart(Data data, String mode, final double p, final int z, String zName, double min, double max) {
	...
	public static void main(String[] args) {
		try {
			for(int law=0; law<LAWS.length; law++) {
				BarChart[] charts = new BarChart[12];
				
				for(int m=0; m<MODES.length; m++) {
					String mode = MODES[m];
					Data data = new Data(String.format("%s/%s_%s.csv", PATH, LAWS[law], mode));
					charts[m*4+0] = createChart(data, mode, 0.9, 1, "IntGPU", RANGES[law][m][0], RANGES[law][m][1]);
					charts[m*4+1] = createChart(data, mode, 0.9, 2, "Nvidia", RANGES[law][m][0], RANGES[law][m][1]);
					charts[m*4+2] = createChart(data, mode, 0.9, 0, "CPU", RANGES[law][m][0], RANGES[law][m][1]);
					charts[m*4+3] = createChart(data, mode, 0.3, 0, "CPU", RANGES[law][m][0], RANGES[law][m][1]);
				}
